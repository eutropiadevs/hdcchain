...
func NewRootCmd() (*cobra.Command, hdc.EncodingConfig) {
	var (
		config  = hdc.MakeEncodingConfig()
		context = client.Context{}.
			...
			WithHomeDir(hdc.DefaultNodeHome)
	)
...
func initRootCmd(rootCmd *cobra.Command, encoding hdc.EncodingConfig) {
	rootCmd.AddCommand(
		genutilcli.InitCmd(hdc.ModuleBasics, hdc.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(banktypes.GenesisBalancesIterator{}, hdc.DefaultNodeHome),
		genutilcli.GenTxCmd(hdc.ModuleBasics, encoding.TxConfig, banktypes.GenesisBalancesIterator{}, hdc.DefaultNodeHome),
		genutilcli.ValidateGenesisCmd(hdc.ModuleBasics),
		AddGenesisAccountCmd(hdc.DefaultNodeHome),
		AddGenesisWasmMsgCmd(hdc.DefaultNodeHome),
		tmcli.NewCompletionCmd(rootCmd, true),
		testnetCmd(hdc.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
	)

	server.AddCommands(rootCmd, hdc.DefaultNodeHome, appCreatorFunc, appExportFunc, addModuleInitFlags)
	rootCmd.AddCommand(
		rpc.StatusCommand(),
		queryCommand(),
		txCommand(),
		keys.Commands(hdc.DefaultNodeHome),
	)
}
...
func queryCommand() *cobra.Command {
	...
	hdc.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	hdc.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")
...
func appCreatorFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, options servertypes.AppOptions) servertypes.Application {
	...
	return hdc.New(
		...
		hdc.MakeEncodingConfig(),
		options,
		hdc.GetWasmEnabledProposals(),
		wasmOpts,
...
func appExportFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, height int64,
	...
	var app *hdc.App
	if height != -1 {
		app = hdc.New(logger, db, tracer, false, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, hdc.GetWasmEnabledProposals(), emptyWasmOpts)

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = hdc.New(logger, db, tracer, true, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, hdc.GetWasmEnabledProposals(), emptyWasmOpts)
	}
